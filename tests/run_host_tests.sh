...
echo "[1/30] strict syntax: FOC + sensor + ISR + runtime integration"
...
echo "[2/30] sensor/FOC behavioral tests"
"$CC" "${CFLAGS[@]}" -I Src tests/test_foc_sensor.c \
  Src/foc_motor.c Src/foc_motor_data.c Src/motor_sensor.c \
  -o /tmp/test_foc_sensor
/tmp/test_foc_sensor

echo "[3/30] VESC outer PID + MTPA + FW numeric/behavior tests"
...
echo "[7/30] VESC 6.00 configuration wire footprint + safety policy"
...
echo "[13/30] Python tester inventory bootstrap (no HBTS circular dependency)"
python3 tests/test_tester_inventory.py

echo "[14/30] VESC facade command/tester coverage contract"
python3 tests/test_tester_command_coverage.py

echo "[15/30] V12 commissioning + standard-wire regression contract"
python3 tests/test_v12_commissioning_contract.py

echo "[16/30] V13 hardware-log regression contract"
python3 tests/test_v13_hardware_regressions.py

echo "[17/30] V14 latest hardware-log regression contract"
python3 tests/test_v14_latest_hardware_regressions.py

echo "[18/30] V14 post-detect ARM/SET contract"
python3 tests/test_v14_post_detect_arm_contract.py

echo "[19/30] V15 VESC SET/GET + V1 ISR contract"
python3 tests/test_v15_vescflow_v1isr.py

echo "[20/30] V16 serial/main-loop liveness contract"
python3 tests/test_v16_serial_liveness.py

echo "[21/30] V17 units/config/speed-direction contract"
python3 tests/test_v17_units_config_speed_contract.py

echo "[22/30] V18 current/brake/handbrake fast-loop behavior"
...
echo "[28/30] V20 LEFT/position/rotor/VdVq/protocol contract"
python3 tests/test_v20_left_position_rotor_vdq_protocol.py

echo "[29/30] V21 19:23 hardware-log regression contract"
python3 tests/test_v21_hwlog_192318.py

echo "[30/30] LEFT TIM4 encoder phase -> FOC/SVPWM numeric chain"
