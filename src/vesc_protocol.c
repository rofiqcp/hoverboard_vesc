...
static void critical_reply_service(void){if(!critical_reply_valid)return;if(send_payload_internal(critical_reply,critical_reply_len,false)){critical_reply_valid=false;critical_reply_len=0U;}}
__attribute__((unused)) static bool send_payload_critical(const uint8_t*p,uint16_t len){if(send_payload_internal(p,len,false))return true;if(!critical_reply_valid&&p!=NULL&&len>0U&&len<=VESC_PACKET_MAX_PAYLOAD){memcpy(critical_reply,p,len);critical_reply_len=len;critical_reply_valid=true;return true;}++tx_drops;return false;}
...
static void auto_detect_start(bool right)
{
    ...
    /* V22: an already VERIFIED offset set is a valid commissioning prerequisite.
     * Do not launch a second zero-current calibration merely because Apply-All was
     * requested. On V21 this created a short invalid-offset window and the first
     * RIGHT Hall start could be rejected with result_code=START_REJECTED (9). */
    if (MotorControl_CurrentOffsetsValid()) {
        if(RuntimeControl_VescStartSensorDetect(false,MOTOR_SENSOR_HALL_UVW,
                                                detect_current_internal(true,VESC_AUTO_DETECT_CURRENT_A)))
            auto_detect.stage=AUTO_DETECT_RIGHT_HALL;
        else if(RuntimeControl_VescStartSensorDetect(true,MOTOR_SENSOR_ENCODER_AB,
    ...
    } else if (!MotorControl_RequestCurrentOffsetCalibration()) {
        /* Request rejected and there is no verified calibration to fall back to. */
...
static void terminal_command(const uint8_t *data, uint16_t len)
{
    ...
    if (strcmp(c, "hb_boot_status") == 0 || strcmp(c, "hb_sched") == 0) {
        char line[180];
        (void)snprintf(line, sizeof(line),
