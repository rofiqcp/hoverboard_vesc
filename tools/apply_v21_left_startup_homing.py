...
'output zero-duty brake flag')

rep('Src/foc_motor.c',
'''    if (motor == NULL || sample == NULL || output == NULL || motor->m_conf == NULL) return;\n    mc_configuration *conf = motor->m_conf;''',
'''    if (motor == NULL || sample == NULL || output == NULL || motor->m_conf == NULL) return;\n    output->zero_duty_phase_brake = false;\n    mc_configuration *conf = motor->m_conf;''',
'clear zero-duty brake flag')

rep('Src/foc_motor.c',
'''        output_observation_only(motor, sample, output, i_alpha, i_beta);\n        return;\n    }\n\n    int16_t id_target = motor->m_id_set;''',
