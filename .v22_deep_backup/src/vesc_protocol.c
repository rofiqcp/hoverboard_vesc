...
void VescProtocol_ServiceBudget(uint16_t max_rx_bytes) {
    /* VESC Tool's packet_process_thread semaphore gives up the CPU after
     * processing a configurable byte budget per pass. This matches that
     * behaviour for compatibility with the VESC Tool API and is called
     * from vesc_services.c service_packet_process(). */
    (void)max_rx_bytes;
    /* Parser state is updated by DMA + VescProtocol_ProcessReceivedByte.
     * No separate budget accounting needed here — RX runs in the main loop
     * and is naturally bounded by main loop period and DMA ring-buffer size. */
}
